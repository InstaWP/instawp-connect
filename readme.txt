...
- Security: Custom plugin and theme archives copied for Sync (Beta) are now stored under an unguessable filename instead of a predictable one, and are cleaned up after the sync. Updating also removes any predictably-named copies already left behind on the site.
