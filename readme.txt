...
- Security: Custom plugin and theme archives copied for Sync (Beta) are now stored under an unguessable filename instead of a predictable one, and are cleaned up after the sync. Updating also removes any predictably-named copies already left behind on the site; if one of those still had a sync waiting to run, re-upload that plugin or theme on the source site to sync it.
