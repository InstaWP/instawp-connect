    ...
    <?php foreach ( $events as $event ) : ?>
        ...
        <tr>
            ...
            <div class="flex flex-col items-center">
                <div class="bg-[#005e54] text-primary-900 text-sm font-medium mr-2 px-3 py-1 rounded-full synced_status <?php echo esc_attr( $event_status . ' ' . $label_class ); ?>" <?php echo esc_html( $label_attributes ); ?>><?php echo esc_html( ucfirst( $event_status ) ); ?></div>
                <?php if ( isset( $event->log ) && $event->log !== '' ) : ?>
                    ...
                    <div>
                <?php endif ?>
                <!-- <div class="py-1 px-2 inline-block rounded-full text-primary-900 font-medium synced_status <?php //echo $event_status.' '.$label_class; ?>" <?php //echo $label_attributes; ?>>
                    <?php //echo esc_html( ucfirst($event_status) ); ?>
                </div> -->
                    <?php // if( $event && $event->synced_message !='' && $event_status == 'error' ) { ?>
            ...
            <!-- <td class="whitespace-nowrap cursor-pointer  text-center px-6 py-6 font-medium text-sm text-primary-900">
                ...
                <?php echo ( $event_status !== 'completed' ) ? '<button type="button" id="btn-sync-'.esc_attr( $event->id ).'" data-id="'. esc_attr( $event->id ).'" class="two-way-sync-btn btn-single-sync"><span>Sync changes </span></button><span class="sync-success"></span>' : '<p class="sync_completed">Synced</p>';  ?>
            </td> -->
        </tr>
    <?php endforeach; ?>
    <?php else : ?> 
