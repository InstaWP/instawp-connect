    ...
    <div class="p-8 space-y-6">
        <div class="flex items-center gap-2 flex-col">
            ...
            <h2 class="text-sm text-grayCust-550">
                <?= esc_html__( 'Upgrade your site management plan for better site security and more', 'instawp-connect' ); ?>
            </h2>
        </div>
        <?php if ( $disconnected ) { ?>
            <div class="flex items-center flex-wrap justify-between px-4 py-3 border border-grayCust-650 rounded-lg" style="background: linear-gradient(90deg, rgba(240,242,236,1) 0%, rgba(17,191,133,0.1) 76%, rgba(238,223,92,0.1) 100%);">
                <div class="flex items-center gap-3 flex-wrap">
                    ...
                    <span class="text-base text-grayCust-800"><?= esc_html__( 'Your website is no longer protected. Subscribe today to Pro Plan!', 'instawp-connect' ); ?></span>
                </div>
                <button class="bg-blueCust-200 text-white px-4 py-2 rounded-lg" id="instawp-protect-site-btn"><?= esc_html__( 'Protect My Site', 'instawp-connect' ); ?></button>
            </div>
        <?php } ?>
