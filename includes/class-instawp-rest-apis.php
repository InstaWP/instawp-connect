...
class InstaWP_Rest_Apis{
     ...
     * @return string|null 
     */
    public function events_receiver($data) { 
        $d = (array) $data;
        echo "<pre>";
        print_r($d);
        die;
    ...
    public function events_receiver1($data) { 
        $dataArr = json_decode($data);
        $response = [];
        if(isset($dataArr->encrypted_contents)){
            if(!empty($dataArr->encrypted_contents) && is_array($dataArr->encrypted_contents)){
                foreach($dataArr->encrypted_contents as $v){
                    $status = '';
                    #Post trash
    ...
    public function sync_history_save($v = null, $status = null){
        ...
        $data = [
            ...
            'encrypted_contents' => $v->event_type,
