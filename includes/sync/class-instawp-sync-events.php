...
class InstaWP_Sync_Events
{
	...
	public function save_widget_action( $id, $sidebar_id, $request, $creating ) {
		...
		$rel          = InstaWP_Sync_DB::getByInCondition( INSTAWP_DB_TABLE_EVENTS, array( 'event_slug' => 'widget_block' ) );
    ...
    public function customizeSaveAfter($manager) {
        $mods = get_theme_mods();
        $data['custom_logo'] = array(
            'id'  => $mods['custom_logo'],
            'url' => wp_get_attachment_url($mods['custom_logo']),
        );
        $data['background_image'] = array(
