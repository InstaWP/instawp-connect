...
class InstaWP_Sync_User {
	...
	public function user_register( $user_id, $userdata ) {
		...
		$details = [ 'user_data' => $userdata, 'user_meta' => get_user_meta( $user_id ), 'db_prefix' => InstaWP_Sync_DB::prefix() ];
	...
	public function profile_update( $user_id, $old_user_data, $userdata ) {
		...
		if ( ! empty( $userdata ) && isset( $_POST['submit'] ) ) {
			...
			$userData = InstaWP_Sync_DB::getByInCondition( InstaWP_Sync_DB::prefix() . 'users', [ 'ID' => $user_id ] );
			if ( isset( $userData[0] ) ) {
				$details = [
		...
		if ( $v->event_slug === 'profile_update' && ! empty( $user_data ) ) {
			if ( $user ) {
				...
				if ( is_wp_error( $user_id ) ) {
					...
					InstaWP_Sync_DB::update( $user_table, [ 'user_pass' => $user_pass ], [ 'ID' => $user_id ] );
