...
class InstaWP_Restore_DB_PDO_Mysql_Method
{
    private $db;
    ...
    public function test_db() {
        ...
        $test_table_new = uniqid('instawp_test_tables_');
        $columns = '(test_id int)';
        $test_table = $this->db->exec("CREATE TABLE IF NOT EXISTS $test_table_new $columns");

        if ( $test_table !== false ) {
            $instawp_plugin->restore_data->write_log('The test to create table succeeds.','notice');
            $test_table = $this->db->exec("INSERT INTO $test_table_new (`test_id`) VALUES ('123')");
            if ( $test_table !== false ) {
                $instawp_plugin->restore_data->write_log('The test to insert into table succeeds.','notice');
                $test_table = $this->db->exec("DROP TABLE IF EXISTS $test_table_new");
                if ( $test_table !== false ) {
                    $instawp_plugin->restore_data->write_log('The test to drop table succeeds.','notice');
                    return true;
                }
                else {
                    $error = $this->db->errorInfo();
                    $instawp_plugin->restore_data->write_log('Unable to drop table. The reason is '.$error[2],'warning');
                    return false;
                }
            }
            else {
                $error = $this->db->errorInfo();
                $instawp_plugin->restore_data->write_log('Unable to insert into table. The reason is '.$error[2],'warning');
                return false;
            }
        }
        else {
            $error = $this->db->errorInfo();
            $instawp_plugin->restore_data->write_log('Unable to create table. The reason is '.$error[2],'warning');
            return false;
        }
    }
        ...
        if ( $max_allowed_packet ) {
            ...
            if ( is_array($max_allowed_packet) && isset($max_allowed_packet[0]) && isset($max_allowed_packet[0][0]) ) {
                if ( $max_allowed_packet[0][0] < 1048576 ) {
                    $instawp_plugin->restore_data->write_log('warning: max_allowed_packet less than 1M :'.size_format($max_allowed_packet[0][0],2),'notice');
                }
                elseif ( $max_allowed_packet[0][0] < 33554432 ) {
                    $instawp_plugin->restore_data->write_log('max_allowed_packet less than 32M :'.size_format($max_allowed_packet[0][0],2),'notice');
                }
                $this->max_allow_packet = $max_allowed_packet[0][0];
                $instawp_plugin->restore_data->write_log( $this->max_allow_packet,'notice');
            }
            else {
                $instawp_plugin->restore_data->write_log('get max_allowed_packet failed','notice');
    ...
    public function execute_sql( $query ) {
        ...
        if ( $this->skip_query > 10 ) {
            if ( strlen($query) > $this->max_allow_packet ) {
                $instawp_plugin->restore_data->write_log('skip query size:'.size_format(strlen($query)), 'Warning');
                return ;
            }
        }
        ...
        if ( $this->db->exec($query) === false ) {
            ...
            $instawp_plugin->restore_data->write_log($query.' query: [' . implode('][', $info) . ']', 'Warning');

            if ( $info[1] == 2006 ) {
                if ( strlen($query) > $this->max_allow_packet ) {
                    $this->skip_query++;
                    $instawp_plugin->restore_data->write_log('max_allow_packet too small:'.size_format($this->max_allow_packet).' query size:'.size_format(strlen($query)), 'Warning');
                }
                $ret = $this->connect_db();
                if ( $ret['result'] == INSTAWP_FAILED ) {
                    $instawp_plugin->restore_data->write_log('reconnect failed', 'Warning');
                }
                else {
                    $instawp_plugin->restore_data->write_log('reconnect succeed', 'Warning');
                }
            }
        }
    }
