...
class InstaWP_Rest_Api {
	...
	public function set_config( WP_REST_Request $request ) {
		...
		try {
			...
			// Generate API Key
			$response = instawp_create_api_connect( $api_key, $jwt, $config );
			if ( false === $response ) {
				return $this->send_response(
					array(
						'status'  => false,
		...
		return $this->send_response(
			array(
				...
				'message' => $removed
					...
					: __( 'Migration not recorded as ended; agent left in place.', 'instawp-connect' ),
			)
		);
	}
	...
	public function validate_api_request( WP_REST_Request $request, $option = '', $match_key = false ) {
		...
		if ( ! empty( $option ) && ! $this->is_enabled( $option ) ) {
			$message = sprintf(
				'Setting is disabled! Please enable %s Option from InstaWP Connect <a href="%s" target="_blank">Remote Management settings</a> page.',
