...
include_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-instawp-backup.php';

class InstaWP_RestoreSite {
	...
	public function restore( $option, $files ) {
		...
		if ( isset( $option['has_child'] ) ) {
			...
			$root_path   = $instawp_plugin->get_backup_folder();
			...
			$backup      = $instawp_plugin->restore_data->get_backup_data();
			$backup_item = new InstaWP_Backup_Item( $backup );
			$local_path  = $instawp_plugin->get_backup_folder();
			...
			if ( $is_type_db ) {
				...
				} elseif ( isset( $option['root_flag'] ) ) {
					if ( $option['root_flag'] == INSTAWP_BACKUP_ROOT_WP_CONTENT ) {
						$root_path = $this->transfer_path( WP_CONTENT_DIR );
					} elseif ( $option['root_flag'] == INSTAWP_BACKUP_ROOT_CUSTOM ) {
						$root_path = $this->transfer_path( WP_CONTENT_DIR . DIRECTORY_SEPARATOR . InstaWP_Setting::get_backupdir() );
					} elseif ( $option['root_flag'] == INSTAWP_BACKUP_ROOT_WP_ROOT ) {
						$root_path = $this->transfer_path( ABSPATH );
					} elseif ( $option['root_flag'] == INSTAWP_BACKUP_ROOT_WP_UPLOADS ) {
						$upload_dir  = wp_upload_dir();
						$upload_path = $upload_dir['basedir'];

						$root_path = $this->transfer_path( $upload_path );
					} elseif ( $option['root_flag'] == 'instawp_mu_upload' ) {
						if ( $option['overwrite'] ) {
						...
						$instawp_plugin->restore_data->write_log( 'restore root path:' . $root_path, 'notice' );
					}
				}

				$root_path = rtrim( $root_path, '/' );
				$root_path = rtrim( $root_path, DIRECTORY_SEPARATOR );
				//$instawp_plugin->restore_data->write_log('root_path:' . $root_path.' root_flag:'.$option['root_flag'], 'notice');
				$exclude_path[] = $this->transfer_path( WP_CONTENT_DIR . DIRECTORY_SEPARATOR . InstaWP_Setting::get_backupdir() );
				...
				$instawp_plugin->restore_data->write_log( 'restore from files:' . json_encode( $all_files ), 'notice' );

				if ( isset( $option['wp_core'] ) && isset( $option['is_migrate'] ) && is_multisite() ) {
					@rename( get_home_path() . '.htaccess', get_home_path() . '.htaccess_old' );
				}

				if ( isset( $option['file_type'] ) && $option['file_type'] == 'wp-content' && ( ABSPATH != str_replace( 'wp-content', '', WP_CONTENT_DIR ) ) ) {
					...
					if ( $option['file_type'] == 'themes' ) {
						if ( isset( $option['remove_themes'] ) ) {
							foreach ( $option['remove_themes'] as $slug => $themes ) {
								...
								$instawp_plugin->restore_data->write_log( 'remove ' . get_theme_root() . DIRECTORY_SEPARATOR . $slug, 'notice' );
						...
						if ( isset( $option['remove_plugin'] ) ) {
							foreach ( $option['remove_plugin'] as $slug => $plugin ) {
								...
								$instawp_plugin->restore_data->write_log( 'remove ' . WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . $slug, 'notice' );
				...
				if ( isset( $option['wp_core'] ) && isset( $option['is_migrate'] ) ) {
					if ( $option['is_migrate'] == 1 && ! is_multisite() ) {
						if ( function_exists( 'save_mod_rewrite_rules' ) ) {
							if ( file_exists( get_home_path() . '.htaccess' ) ) {
								...
								if ( preg_match( '#AddHandler application/x-httpd-php.*#', $htaccess_data, $matcher ) ) {
									...
									$instawp_plugin->restore_data->write_log( 'find php selector:' . $line, 'notice' );
								} elseif ( preg_match( '#AddHandler application/x-httpd-ea-php.*#', $htaccess_data, $matcher ) ) {
						...
						InstaWP_Setting::update_option( 'instawp_migrate_status', 'completed' );
					}
				}
			}
			...
			if ( array_key_exists( $path, $tested_paths ) ) {
				...
				if ( ! wp_mkdir_p( $path ) ) {
					...
					/* translators: %s: directory path */
						__( 'Unable to create directory %s. Is its parent directory writable by the server?', 'instawp-connect' ),
						esc_html( $error_path )
					);
				}
