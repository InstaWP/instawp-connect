...
class InstaWP_DB_Method
{
            ...
            if ( class_exists('PDO') ) {
                ...
                if ( array_search('pdo_mysql',$extensions) ) {
                    ...
                    return array(
						...
						'error'  => 'The pdo_mysql extension is not detected. Please install the extension first or choose wpdb option for Database connection method.',
					);
                }
            }
            else {
    ...
    public function check_db( $fcgi ) {
        ...
        elseif ( $this->type == 'wpdb' ) {
            return $this->check_db_wpdb($fcgi);
        }

        return array(
			'result' => INSTAWP_FAILED, 
			'error'  => 'db handle type not found.',
		);
    }
    ...
    public function check_max_allowed_packet() {
        ...
        if ( $max_allowed_packet < 1048576 ) {
            $instawp_plugin->instawp_log->WriteLog('warning: max_allowed_packet less than 1M :'.size_format($max_allowed_packet,2),'notice');
        }
        elseif ( $max_allowed_packet < 33554432 ) {
            $instawp_plugin->instawp_log->WriteLog('max_allowed_packet less than 32M :'.size_format($max_allowed_packet,2),'notice');
        }
    }
}