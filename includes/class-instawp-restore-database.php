...
require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-instawp-zipclass.php';
require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-instawp-backup-database.php';
require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-instawp-backup-site.php';
require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-instawp-setting.php';

class InstaWP_RestoreDB {
	...
	public function restore( $path, $sql_file, $options ) {
		...
		include_once INSTAWP_PLUGIN_DIR . '/includes/class-instawp-restore-db-method.php';
		$this->db_method = new InstaWP_Restore_DB_Method();
		$this->db_method->set_skip_query( 0 );

		global $instawp_plugin;
		if ( file_exists( $path . $sql_file ) ) {
			if ( ! isset( $options['skip_backup_old_database'] ) ) {
				$instawp_plugin->restore_data->write_log( 'Backing up current site\'s database.', 'notice' );
				...
				if ( $result['result'] == INSTAWP_FAILED ) {
					$instawp_plugin->restore_data->write_log( 'Backing up current site\'s database failed.error:' . $result['error'], 'warning' );
		...
		if ( file_exists( $file_tables_rows_data ) && file_exists( $file_tables_rows ) ) {
			...
			foreach ( $tables_rows_data as $table_name => $table_rows ) {
				foreach ( $table_rows as $table_row ) {
					...
					$where_clause_arr    = array_map( function ( $val, $key ) {
						return "`$key`='$val'";
					}, $where_clause, array_keys( $where_clause ) );
...
//			unlink( $file_tables_rows );
	...
	private function instawp_fix_siteurl_home() {
		...
		if ( $this->old_site_url != $this->new_site_url ) {
			...
			$instawp_plugin->restore_data->write_log( $update_query, 'notice' );
			$instawp_plugin->restore_data->write_log( 'update query len:' . strlen( $update_query ), 'notice' );
	...
	private function execute_extra_sql_file( $file, $options ) {
		global $instawp_plugin;
		$instawp_plugin->restore_data->write_log( 'Start import additional sql file.', 'notice' );
		$dbhost     = '';
		$dbuser     = '';
		$dbpassword = '';
		$dbname     = '';
		...
		$restore_extra_db = new InstaWP_Restore_DB_Extra( $dbhost, $dbuser, $dbpassword, $dbname );
	...
	private function enable_plugins() {
		...
		$plugin_list   = array();
		$plugin_list[] = 'instawp-connect/instawp-connect.php';
		$plugin_list   = apply_filters( 'instawp_enable_plugins_list', $plugin_list );
	...
	private function execute_sql_file( $file, $option ) {
		global $wpdb, $instawp_plugin;

		$instawp_plugin->restore_data->write_log( 'Start import sql file.', 'notice' );
		...
		if ( $this->is_mu && isset( $option['site_id'] ) ) {
			...
			$instawp_plugin->restore_data->write_log( 'old site prefix:' . $this->old_prefix, 'notice' );
			$this->old_site_url = $option['site_url'];
			$instawp_plugin->restore_data->write_log( 'old site url:' . $this->old_site_url, 'notice' );
			$this->old_home_url = $option['home_url'];
			$instawp_plugin->restore_data->write_log( 'old home url:' . $this->old_home_url, 'notice' );
		...
		while ( ! feof( $sql_handle ) ) {
				...
				if ( $endWith == ';' && preg_match( '- # -', $line ) ) {
					...
					if ( empty( $this->content_url ) && preg_match( '# content_url: (.*?) #', $line, $matcher ) ) {
						if ( empty( $this->old_content_url ) ) {
							...
							$instawp_plugin->restore_data->write_log( 'old content url:' . $this->old_content_url, 'notice' );
						}
					}
					if ( empty( $this->upload_url ) && preg_match( '# upload_url: (.*?) #', $line, $matcher ) ) {
						if ( empty( $this->old_upload_url ) ) {
							$this->old_upload_url = $matcher[1];
							$instawp_plugin->restore_data->write_log( 'old upload url:' . $this->old_upload_url, 'notice' );
						}
					}
			...
			if ( $endWith == ';' ) {
				...
				} elseif ( preg_match( '#^\\s*LOCK TABLES#', $query ) ) {
...
//					}
					...
					if ( $this->replace_table_execute_sql( $query, $current_old_table ) === false ) {
						$instawp_plugin->restore_data->write_log( 'Restore ' . basename( $file ) . ' error at line ' . $line_num . ',' . PHP_EOL . 'errorinfo: [' . implode( '][', $this->db_method->errorInfo() ) . ']', 'Warning' );
						$query = '';
						continue;
					}
				} else {
		...
		InstaWP_Setting::import_json_to_setting( $this->current_setting );
		do_action( 'instawp_reset_schedule' );
		do_action( 'instawp_do_after_restore_db' );
	...
	private function lock_table( $query ) {
		...
		if ( ! empty( $this->old_prefix ) && $this->old_prefix != $this->new_prefix ) {
			if ( preg_match( '/^\s*LOCK TABLES +\`?([^\`]*)\`?/i', $query, $matches ) ) {
				...
				//$new_table_name=$this->new_prefix.substr($table_name,strlen($this->old_prefix));
				$instawp_plugin->restore_data->write_log( 'lock replace table:' . $table_name . ' to :' . $new_table_name, 'notice' );
	...
	private function create_table( $query, &$current_old_table ) {
		...
		if ( ! empty( $this->old_prefix ) && $this->old_prefix != $this->new_prefix ) {
			...
			$query = str_replace( $table_name, $new_table_name, $query );
			$instawp_plugin->restore_data->write_log( 'Create table ' . $new_table_name, 'notice' );
			$table_name = $new_table_name;
		} else {
			$instawp_plugin->restore_data->write_log( 'Create table ' . $table_name, 'notice' );
		}
		...
		if ( preg_match( '/ENGINE=([^\s;]+)/', $query, $matches ) ) {
			...
			if ( $replace_engine !== false ) {
				$instawp_plugin->restore_data->write_log( 'create table replace engine:' . $engine . ' to :' . $replace_engine, 'notice' );
		...
		if ( preg_match( '/CHARSET ([^\s;]+)/', $query, $matches ) || preg_match( '/CHARSET=([^\s;]+)/', $query, $matches ) ) {
			...
			if ( $replace_charset !== false ) {
				$instawp_plugin->restore_data->write_log( 'create table replace charset:' . $charset . ' to :' . $replace_charset, 'notice' );
			...
			if ( preg_match( '/ COLLATE ([a-zA-Z0-9._-]+)/i', $query, $matches ) ) {
				$collate = $matches[1];
			} elseif ( preg_match( '/ COLLATE=([a-zA-Z0-9._-]+)/i', $query, $matches ) ) {
				$collate = $matches[1];
			}
			...
			if ( ! empty( $collate ) ) {
				...
				if ( $replace_collate !== false ) {
					$instawp_plugin->restore_data->write_log( 'create table replace collate:' . $collate . ' to :' . $replace_collate, 'notice' );
	...
	private function insert( $query ) {
		...
		if ( $pos_mainwp_child !== false && $pos_ws_menu_editor_pro === false ) {
			$instawp_plugin->restore_data->write_log( 'skip insert item: ' . $query, 'notice' );
		} else {
			$this->execute_sql( $query );
		}
	}
	...
	private function drop_table( $query ) {
		...
		if ( ! empty( $this->old_prefix ) && $this->old_prefix != $this->new_prefix ) {
			if ( preg_match( '/^\s*DROP TABLE IF EXISTS +\`?([^\`]*)\`?\s*;/i', $query, $matches ) ) {
				...
				if ( ! empty( $this->old_base_prefix ) && ( substr( $table_name, strlen( $this->old_base_prefix ) ) == 'users' || substr( $table_name, strlen( $this->old_base_prefix ) ) == 'usermeta' ) ) {
					$new_table_name = $this->new_prefix . substr( $table_name, strlen( $this->old_base_prefix ) );
					$instawp_plugin->restore_data->write_log( 'find user table:' . $table_name . ' to :' . $new_table_name, 'notice' );
				} else {
					$new_table_name = $this->new_prefix . substr( $table_name, strlen( $this->old_prefix ) );
				}
		...
		$instawp_plugin->restore_data->write_log( 'Drop table if exist', 'notice' );
		$this->execute_sql( $query );
	}
	...
	private function replace_row( $table_name ) {
		...
		$instawp_plugin->restore_data->write_log( 'Dumping table ' . $table_name . ' is complete. Start replacing row(s).', 'notice' );
		...
		if ( $this->is_mu ) {
			if ( substr( $table_name, strlen( $this->new_prefix ) ) == 'blogs' ) {
				$instawp_plugin->restore_data->write_log( 'update mu blogs', 'notice' );
		...
		$skip_table = false;
		if ( apply_filters( 'instawp_restore_db_skip_replace_tables', $skip_table, $table_name ) ) {
			$instawp_plugin->restore_data->write_log( 'Skip table ' . $table_name, 'Warning' );
		...
		if ( $result && sizeof( $result ) > 0 ) {
			$count = $result[0][0];
			$instawp_plugin->restore_data->write_log( 'Count of rows in ' . $table_name . ': ' . $count, 'notice' );
			if ( $count == 0 ) {
				return;
			}
			...
			for ( $current_row = $start_row; $current_row <= $count; $current_row += $page ) {
				$instawp_plugin->restore_data->write_log( 'Replace the row in ' . $current_row . ' line.', 'notice' );
					...
					foreach ( $rows as $row ) {
						...
						foreach ( $columns as $column ) {
							if ( isset( $column['skip'] ) ) {
								$instawp_plugin->restore_data->write_log( 'skip mediumblob type data', 'notice' );
								continue;
							}
							...
							$skip_row = false;
							if ( apply_filters( 'instawp_restore_db_skip_replace_rows', $skip_row, $table_name, $column['Field'] ) ) {
								continue;
							}
							...
							if ( $type == 'pdo_mysql' ) {
								if ( $update_query == '' ) {
									...
									if ( strlen( $update_query ) > $this->db_method->get_max_allow_packet() ) {
										$instawp_plugin->restore_data->write_log( 'update replace rows', 'notice' );
										$this->execute_sql( $update_query );

										$update_query = '';
									}
								} elseif ( strlen( $temp_query ) + strlen( $update_query ) > $this->db_method->get_max_allow_packet() ) {
									$instawp_plugin->restore_data->write_log( 'update replace rows', 'notice' );
									$this->execute_sql( $update_query );
		...
		$instawp_plugin->restore_data->write_log( 'finish replace rows', 'notice' );
	}
	...
	public function replace_string_v2( $old_string ) {
		...
		} elseif ( 0 === stripos( $this->new_site_url, 'http://' ) || stripos( $this->new_site_url, 'http:\/\/' ) ) {
			$new_url_use_https = false;
		}
