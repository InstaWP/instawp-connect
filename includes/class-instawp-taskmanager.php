...
class InstaWP_taskmanager {
	public static function new_backup_task( $option, $type, $action = 'backup' ) {
		...
		InstaWP_Setting::update_task( $id, $task );
		$ret['result']  = 'success';
		$ret['task_id'] = $task['id'];
	...
	public static function delete_ready_task() {
		...
		if ( ! empty( $delete_ids ) ) {
			InstaWP_Setting::update_option( 'instawp_task_list', $tasks );
		}
	}
	...
	public static function get_backup_tasks_progress( $task_id ) {
		...
		if ( array_key_exists( $task_id, $tasks ) ) {
			...
			// $next_resume_time=InstaWP_Schedule::get_next_resume_time($task['id']);
			...
			if ( isset( $task['data'][ $ret['type'] ]['sub_job'][ $ret['doing'] ]['progress'] ) ) {
				$ret['descript'] = __( $task['data'][ $ret['type'] ]['sub_job'][ $ret['doing'] ]['progress'], 'instawp-connect' );
			} else {
				$ret['descript'] = '';
			}
	...
	public static function update_backup_main_task_progress( $task_id, $job_name, $progress, $finished, $job_data = array() ) {
		...
		if ( $task !== false ) {
			...
			//$task['data'][ $job_name ]['doing'] = $progress;

			//$task['data'][$job_name][ 'current_upload' ] = $message;
			InstaWP_Setting::update_task( $task_id, $task );
		}
	}

	public static function update_backup_main_current_upload( $task_id, $message = array() ) {
		$task = self::get_task( $task_id );
		if ( $task !== false ) {
			$task['data']['current_upload'] = $message;
	...
	public static function update_file_desc( $task_id, $message = '' ) {
		$task     = self::get_task( $task_id );
		$descript = $task['task_info']['descript'];
		//$task['task_info']['api_descript'] = $api_descript. ' '.$message;
		$task['task_info']['descript'] = $descript . ' ' . $message;
	...
	public static function update_download_cache( $backup_id, $cache ) {
		...
		$options                        = get_option( 'instawp_download_cache', $default );
		$options[ $backup_id ]['cache'] = $cache;
		InstaWP_Setting::update_option( 'instawp_download_cache', $options );
	}
	...
	public static function update_task( $task ) {
		if ( empty( $task_id = InstaWP_Setting::get_args_option( 'id', (array) $task ) ) ) {
			return false;
		}
	...
	public static function delete_task( $id ) {
		InstaWP_Setting::delete_task( $id );
	}
	...
	public static function delete_all_task() {
		InstaWP_Setting::delete_option( 'instawp_task_list' );
	}
	...
	public static function new_download_task_v2( $file_name ) {
		...
		wp_cache_delete( 'instawp_download_task_v2', 'options' );

		$options = get_option( 'instawp_download_task_v2', $default );
		...
		InstaWP_Setting::update_option( 'instawp_download_task_v2', $options );
	...
	public static function is_there_any_incomplete_task_ids() {
		...
		foreach ( InstaWP_Setting::get_tasks() as $task ) {
			if (
				...
				isset( $task['action'] ) && $task['action'] != 'download' &&
	...
	public static function get_files_to_be_uploaded( $task_id ) {

		$files_to_be_uploaded = [];

		foreach ( InstaWP_taskmanager::get_task_backup_data( $task_id ) as $key => $data ) {
			foreach ( InstaWP_Setting::get_args_option( 'zip_files_path', $data, array() ) as $zip_file ) {
				$files_to_be_uploaded[] = $zip_file;
			}
		}
	...
	public static function get_data( $task_id, $key = '', $default = '' ) {
		...
		return InstaWP_Setting::get_args_option( $key, $migrate_task, $default );
	}

	public static function set_data( $task_id, $key = '', $value = '' ) {
		...
		$migrate_task[ $key ] = $value;

		self::update_task( $migrate_task );
