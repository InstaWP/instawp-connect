...
define( 'INSTAWP_BACKUP_TYPE_DB', 'backup_db' );
define( 'INSTAWP_BACKUP_TYPE_THEMES', 'backup_themes' );
define( 'INSTAWP_BACKUP_TYPE_PLUGIN', 'backup_plugin' );
define( 'INSTAWP_BACKUP_TYPE_UPLOADS', 'backup_uploads' );
define( 'INSTAWP_BACKUP_TYPE_UPLOADS_FILES', 'backup_uploads_files' );
//define('INSTAWP_BACKUP_TYPE_UPLOADS_FILES_OTHER','backup_uploads_files_other');
define( 'INSTAWP_BACKUP_TYPE_CONTENT', 'backup_content' );
define( 'INSTAWP_BACKUP_TYPE_CORE', 'backup_core' );
define( 'INSTAWP_BACKUP_TYPE_OTHERS', 'backup_others' );
define( 'INSTAWP_BACKUP_TYPE_MERGE', 'backup_merge' );

define( 'INSTAWP_BACKUP_ROOT_WP_CONTENT', 'wp-content' );
define( 'INSTAWP_BACKUP_ROOT_CUSTOM', 'custom' );
define( 'INSTAWP_BACKUP_ROOT_WP_ROOT', 'root' );
define( 'INSTAWP_BACKUP_ROOT_WP_UPLOADS', 'uploads' );
...
class InstaWP_Backup_Task {
	...
	public function __construct( $task_id = false, $task = false ) {
		...
		$this->backup_type_collect[ INSTAWP_BACKUP_TYPE_DB ]            = 1;
		$this->backup_type_collect[ INSTAWP_BACKUP_TYPE_THEMES ]        = 1;
		$this->backup_type_collect[ INSTAWP_BACKUP_TYPE_PLUGIN ]        = 1;
		$this->backup_type_collect[ INSTAWP_BACKUP_TYPE_UPLOADS ]       = 1;
		$this->backup_type_collect[ INSTAWP_BACKUP_TYPE_UPLOADS_FILES ] = 1;
		//$this->backup_type_collect[INSTAWP_BACKUP_TYPE_UPLOADS_FILES_OTHER]=1;
		$this->backup_type_collect[ INSTAWP_BACKUP_TYPE_CONTENT ] = 1;
		$this->backup_type_collect[ INSTAWP_BACKUP_TYPE_CORE ]    = 1;
		$this->backup_type_collect[ INSTAWP_BACKUP_TYPE_OTHERS ]  = 1;
		$this->backup_type_collect[ INSTAWP_BACKUP_TYPE_MERGE ]   = 1;

		add_filter( 'instawp_set_backup', array( $this, 'instawp_set_backup' ), 10 );
//		add_filter( 'instawp_exclude_plugins', array( $this, 'exclude_plugins' ), 20 );
		add_filter( 'instawp_get_backup_exclude_regex', array( $this, 'get_backup_exclude_regex' ), 10, 2 );
	}
	...
	public function get_backup_exclude_regex( $exclude_regex, $args ) {
		...
		$excluded_paths    = InstaWP_Setting::get_args_option( 'excluded_paths', $migrate_settings, [] );
		...
		if ( $backup_type == INSTAWP_BACKUP_TYPE_UPLOADS || $backup_type == INSTAWP_BACKUP_TYPE_UPLOADS_FILES ) {
			...
			$exclude_regex[] = '#^' . preg_quote( $this->transfer_path( untrailingslashit( ABSPATH ) . DIRECTORY_SEPARATOR ), DIRECTORY_SEPARATOR ) . 'instawp-.*\.php#';
			$exclude_regex[] = '#^' . preg_quote( $this->transfer_path( untrailingslashit( ABSPATH ) . DIRECTORY_SEPARATOR ), DIRECTORY_SEPARATOR ) . 'instawp-.*\.zip#';
		...
		if ( isset( $args['excluded_paths'] ) && is_array( $args['excluded_paths'] ) ) {
			$excluded_paths = array_merge( $excluded_paths, $args['excluded_paths'] );
		}

		foreach ( $excluded_paths as $excluded_path ) {
			$exclude_regex[] = '#^' . preg_quote( $this->transfer_path( untrailingslashit( ABSPATH ) . DIRECTORY_SEPARATOR . $excluded_path ), DIRECTORY_SEPARATOR ) . '#';
		}
	...
	public function new_backup_task( $options, $type, $action = 'backup' ) {
		...
		if ( isset( $options['remote'] ) ) {
			if ( $options['remote'] == '1' ) {
				if ( isset( $options['remote_options'] ) ) {
					...
					$this->task['options']['remote_options'] = InstaWP_Setting::get_remote_options();
				}
			} else {
		...
		$this->task['options']['remote_options'] = apply_filters( 'instawp_set_remote_options', $this->task['options']['remote_options'], $options );
		...
		if ( isset( $options['backup_prefix'] ) && ! empty( $options['backup_prefix'] ) ) {
			...
			if ( isset( $general_setting['options']['instawp_common_setting']['domain_include'] ) && $general_setting['options']['instawp_common_setting']['domain_include'] ) {
				$check_addon = apply_filters( 'instawp_check_setting_addon', 'not_addon' );
				if ( isset( $general_setting['options']['instawp_common_setting']['backup_prefix'] ) && $check_addon == 'addon' ) {
					$backup_prefix = $general_setting['options']['instawp_common_setting']['backup_prefix'];
				} else {
		...
		$this->task['options']['file_prefix'] = apply_filters( 'instawp_backup_file_prefix', $this->task['options']['file_prefix'], $backup_prefix, $this->task['id'], $this->task['status']['start_time'] );
		...
		$this->task['options']['backup_options']['ismerge'] = apply_filters( 'instawp_set_backup_ismerge', $this->task['options']['backup_options']['ismerge'], $options );
		...
		$this->task['options']['backup_options']['compress'] = InstaWP_Setting::get_option( 'instawp_compress_setting' );
		$this->task['options']['backup_options']['dir']      = InstaWP_Setting::get_backupdir();
		...
		$this->task['data']['download']['doing']           = '';
		$this->task['data']['download']['finished']        = 0;
	...
	public function new_download_task( $parameters = array() ) {
		...
		$this->task['status']['str']            = 'ready';
		$this->task['status']['resume_count']   = 0;
		$this->task['options']['log_file_name'] = $id . '_restore';
		$general_setting                        = InstaWP_Setting::get_setting( true, "" );
		$log                                    = new InstaWP_Log();
		//$log->CreateLogFile($this->task['options']['log_file_name'], 'no_folder', 'download');
		$this->task['data']['doing']                       = 'download';
		$this->task['data']['backup']['doing']             = '';
		$this->task['data']['backup']['finished']          = 0;
		$this->task['data']['backup']['progress']          = 0;
		$this->task['data']['backup']['job_data']          = array();
	...
	public function set_backup( $backup, $task_type = '', $options = array() ) {
		...
		if ( is_string( $backup ) ) {
			...
			} elseif ( $backup == INSTAWP_BACKUP_TYPE_PLUGIN ) {

				$backup_data['root_flag']              = INSTAWP_BACKUP_ROOT_WP_CONTENT;
				$backup_data['prefix']                 = $this->get_prefix() . '_backup_plugin';
				$backup_data['plugin_subpackage']      = isset( $backup_data['compress']['subpackage_plugin_upload'] ) && $backup_data['compress']['subpackage_plugin_upload'];
			...
			} elseif ( $backup == INSTAWP_BACKUP_TYPE_CONTENT ) {
			...
			} elseif ( $backup == INSTAWP_BACKUP_TYPE_MERGE ) {
				//$backup_data['root_path']=WP_CONTENT_DIR.DIRECTORY_SEPARATOR.$this->task['options']['backup_options']['dir'];
				$backup_data['root_flag']              = INSTAWP_BACKUP_ROOT_CUSTOM;
				$file_name                             = $this->get_prefix() . '_backup_all';
				$file_name                             = apply_filters( 'instawp_set_incremental_backup_file_name', $file_name, $this->get_prefix(), $task_type );
	...
	public function get_need_backup() {
		$this->task              = InstaWP_taskmanager::get_task( $this->task['id'] );
		$backup                  = false;
		...
		foreach ( $this->task['options']['backup_options']['backup'] as $key => $backup_data ) {
			...
			if ( $backup_data['finished'] == 0 ) {
				if ( $backup === false ) {
					add_filter( 'instawp_get_need_backup_files', array( $this, 'instawp_get_need_backup_files' ), 10 );
					$backup_data = apply_filters( 'instawp_get_need_backup_files', $backup_data );
		...
		if ( $i_count_of_backup > 0 ) {
			...
			InstaWP_taskmanager::update_backup_main_task_progress( $this->task['id'], 'backup', $i_progress * $i_finished_backup_count, 0 );

			if ( $i_finished_backup_count >= $i_count_of_backup ) {
				if ( $this->task['options']['backup_options']['ismerge'] == 1 ) {
					if ( ! array_key_exists( INSTAWP_BACKUP_TYPE_MERGE, $this->task['options']['backup_options']['backup'] ) ) {
						$this->set_backup( INSTAWP_BACKUP_TYPE_MERGE, $this->task['type'] );
						InstaWP_Setting::update_task( $this->task['id'], $this->task );
						$backup = $this->task['options']['backup_options']['backup'][ INSTAWP_BACKUP_TYPE_MERGE ];
					}
				}
			}
		}
	...
	public function get_need_backup_files_size( $backup_data ) {
		if ( isset( $backup_data['files'] ) && ! empty( $backup_data['files'] ) ) {
			...
			return $this->get_file_list( $backup_data['files_root'], $backup_data['exclude_regex'], $backup_data['include_regex'], $this->task['options']['backup_options']['compress']['exclude_file_size'], $backup_data );
		}
	}

	public function instawp_get_need_backup_files( $backup_data ) {
		if ( ! isset( $backup_data['dump_db'] ) ) {
			if ( array_key_exists( $backup_data['key'], $this->backup_type_collect ) ) {
				...
				$backup_data['files'] = apply_filters( 'instawp_get_custom_need_backup_files', $backup_data['files'], $backup_data, $this->task['options']['backup_options']['compress'] );
			}

			$need = false;
			add_filter( 'instawp_need_backup_files_update', array( $this, 'need_backup_files_update' ), 10, 2 );
			if ( apply_filters( 'instawp_need_backup_files_update', $need, $backup_data ) ) {
				$this->task['options']['backup_options']['backup'][ $backup_data['key'] ] = $backup_data;
				InstaWP_Setting::update_task( $this->task['id'], $this->task );
			}
		}
	...
	public function update_backup_result( $backup_data, $result ) {
		...
		if ( array_key_exists( $backup_data['key'], $this->task['options']['backup_options']['backup'] ) ) {
			...
			add_filter( 'instawp_backup_update_result', array( $this, 'instawp_backup_update_result' ), 10, 2 );
			$result                                                                             = apply_filters( 'instawp_backup_update_result', $result, $backup_data );
			$this->task['options']['backup_options']['backup'][ $backup_data['key'] ]['result'] = $result;

			InstaWP_taskmanager::update_task_options( $this->task['id'], 'backup_options', $this->task['options']['backup_options'] );

			if ( $result['result'] == INSTAWP_FAILED ) {
				InstaWP_taskmanager::update_backup_task_status( $this->task['id'], false, 'error', false, false, $result['error'] );
		...
		if ( $i_finished_backup_count >= $i_count_of_backup ) {
			InstaWP_taskmanager::update_backup_main_task_progress( $this->task['id'], 'backup', 100, 1 );
		}
		//InstaWP_Setting::update_task($this->task['id'],$this->task);
	}

	public function instawp_backup_update_result( $result, $backup_data ) {
	...
	public function get_backup_result() {
		...
		foreach ( $this->task['options']['backup_options']['backup'] as $backup_data ) {
			if ( $this->task['options']['backup_options']['ismerge'] == 1 ) {
				if ( INSTAWP_BACKUP_TYPE_MERGE == $backup_data['key'] ) {
					$ret = $backup_data['result'];
					if ( $ret['result'] !== INSTAWP_SUCCESS ) {
						return $ret;
					}
	...
	public function get_file_list( $root, $exclude_regex, $include_regex, $exclude_file_size ) {
		$files = array();
		$this->getFileLoop( $files, $root, $exclude_regex, $include_regex, $exclude_file_size, true );
	...
	public function get_need_cleanup_files( $all = false ) {
		...
		if ( $this->task['options']['backup_options']['ismerge'] == 1 ) {
			foreach ( $this->task['options']['backup_options']['backup'] as $backup_finished_data ) {
				if ( $all === false ) {
					if ( INSTAWP_BACKUP_TYPE_MERGE == $backup_finished_data['key'] ) {
						continue;
					}
				}

				$files = array_merge( $files, $this->get_backup_file( $backup_finished_data['key'] ) );
				add_filter( 'instawp_get_need_cleanup_files', array( $this, 'instawp_get_need_cleanup_files' ), 10, 2 );
				$files = apply_filters( 'instawp_get_need_cleanup_files', $files, $backup_finished_data );
			}
		}
	...
	public function instawp_get_need_cleanup_files( $files, $backup_finished_data ) {
		if ( INSTAWP_BACKUP_TYPE_PLUGIN == $backup_finished_data['key'] ) {
			$general_setting = InstaWP_Setting::get_setting( true, "" );
			if ( isset( $general_setting['options']['instawp_compress_setting']['subpackage_plugin_upload'] ) && ! empty( $general_setting['options']['instawp_compress_setting']['subpackage_plugin_upload'] ) ) {
				if ( $general_setting['options']['instawp_compress_setting']['subpackage_plugin_upload'] ) {
					$packages_files_info = $this->get_packages_files_info( INSTAWP_BACKUP_TYPE_PLUGIN );
					if ( $packages_files_info !== false ) {
						foreach ( $packages_files_info as $file_data ) {
		...
		} elseif ( INSTAWP_BACKUP_TYPE_UPLOADS_FILES == $backup_finished_data['key'] ) {
			$general_setting = InstaWP_Setting::get_setting( true, "" );
			if ( isset( $general_setting['options']['instawp_compress_setting']['subpackage_plugin_upload'] ) && ! empty( $general_setting['options']['instawp_compress_setting']['subpackage_plugin_upload'] ) ) {
				if ( $general_setting['options']['instawp_compress_setting']['subpackage_plugin_upload'] ) {
					$packages_files_info = $this->get_packages_files_info( INSTAWP_BACKUP_TYPE_UPLOADS_FILES );
					if ( $packages_files_info !== false ) {
						foreach ( $packages_files_info as $file_data ) {
							$path = WP_CONTENT_DIR . DIRECTORY_SEPARATOR . $this->task['options']['backup_options']['dir'] . DIRECTORY_SEPARATOR . $file_data['file_name'];
							global $instawp_plugin;
							$instawp_plugin->instawp_log->WriteLog( 'test:' . $path, 'notice' );
							if ( ! in_array( $path, $files ) ) {
								$files[] = $path;
							}
						}
		...
		foreach ( $this->task['options']['backup_options']['backup'] as $backup_data ) {
			if ( ! isset( $backup_data['dump_db'] ) ) {
				if ( array_key_exists( $backup_data['key'], $this->backup_type_collect ) ) {
					...
					$backup_data['files'] = array();
					$backup_files         = apply_filters( 'instawp_get_custom_need_backup_files_size', $backup_data['files'], $backup_data, $this->task['options']['backup_options']['compress'] );
				}
	...
	public function get_backup_task_info( $task_id ) {
		...
		//    $task_id = $bkp_init_opt['task_info']['task_id'];
		// }
		$list_tasks['status']      = InstaWP_taskmanager::get_backup_tasks_status( $task_id );
		$list_tasks['is_canceled'] = InstaWP_taskmanager::is_task_canceled( $task_id );
		$list_tasks['size']        = InstaWP_taskmanager::get_backup_size( $task_id );
		$list_tasks['data']        = InstaWP_taskmanager::get_backup_tasks_progress( $task_id );
		//
		...
		//
		$general_setting = InstaWP_Setting::get_setting( true, "" );
		if ( $general_setting['options']['instawp_common_setting']['estimate_backup'] == 0 ) {
		...
		if ( $list_tasks['status']['str'] == 'ready' ) {
			$list_tasks['task_info']['descript']       = __( 'Ready to backup. Progress: 0%, running time: 0second.', 'instawp-connect' );
		...
		} elseif ( $list_tasks['status']['str'] == 'running' ) {

			if ( $list_tasks['is_canceled'] == false ) {
				//InstaWP_taskmanager::update_backup_main_task_progress($task_id, 'upload', 0, 0);
				if ( $list_tasks['data']['type'] == 'upload' ) {
					update_option( 'instawp_uploaded_data', $list_tasks );
					...
					$current_file = '';
					// $curl_progress = get_option('instawp_curl_progress', '');
					// if (!empty($curl_progress)) {
					//    if (isset($curl_progress['files'])) {
					//       $current_file = $curl_progress['files'];
					//    }
					// }
					$list_tasks['task_info']['speed']        = $speed;
					$list_tasks['task_info']['descript']     = $descript . ' ' . __( 'Progress: ', 'instawp-connect' ) . $list_tasks['task_info']['backup_percent'] . ', ' . __( 'running time: ', 'instawp-connect' ) . $list_tasks['data']['running_time'];
					$list_tasks['task_info']['api_descript'] = $descript;
					...
					if ( ! empty( $curl_progress ) ) {
						if ( isset( $curl_progress['files'] ) ) {
							//$current_file = basename($curl_progress['files']);
						}
					}
					$list_tasks['task_info']['descript'] = $list_tasks['data']['descript'] . ' ' . __( 'Progress: ', 'instawp-connect' ) . $list_tasks['task_info']['backup_percent'] . ', ' . __( 'running time: ', 'instawp-connect' ) . $list_tasks['data']['running_time'] . ' Current Task: ' . $task_id;

					$list_tasks['task_info']['api_descript'] = $list_tasks['data']['descript'];
					...
					//       $url = INSTAWP_API_URL.'/connects/'.$id.'/backup_status';

					//       $body = array(
					//       "task_id" => $task_id,
					//       "type" => 'backup',
					//       "progress" => $list_tasks['task_info']['backup_percent'],
				...
				$list_tasks['task_info']['descript']       = __( 'The backup will be canceled after backing up the current chunk ends.', 'instawp-connect' );
				$list_tasks['task_info']['css_btn_cancel'] = 'pointer-events: none; opacity: 0.4;';
				$list_tasks['task_info']['css_btn_log']    = 'pointer-events: auto; opacity: 1;';
			}
		} elseif ( $list_tasks['status']['str'] == 'wait_resume' ) {
			...
			$list_tasks['task_info']['css_btn_cancel'] = 'pointer-events: auto; opacity: 1;';
			$list_tasks['task_info']['css_btn_log']    = 'pointer-events: auto; opacity: 1;';
		} elseif ( $list_tasks['status']['str'] == 'no_responds' ) {
			if ( $list_tasks['is_canceled'] == false ) {
		...
		} elseif ( $list_tasks['status']['str'] == 'completed' ) {
			...
			//             //$list_tasks = $InstaWP_Curl->upload($backup_info,$list_tasks,$task_id);
			//          }
			//       }
		} elseif ( $list_tasks['status']['str'] == 'upload_completed' ) {
			$list_tasks['task_info']['descript']              = 'Task ' . $task_id . ' completed.';
			$list_tasks['task_info']['css_btn_cancel']        = 'pointer-events: auto; opacity: 1;';
			$list_tasks['task_info']['css_btn_log']           = 'pointer-events: auto; opacity: 1;';
			$list_tasks['task_info']['need_update_last_task'] = false;
		} elseif ( $list_tasks['status']['str'] == 'error' ) {
	...
	public function add_new_backup() {
		...
		$status                                   = InstaWP_taskmanager::get_backup_task_status( $this->task['id'] );
		$backup_data['create_time']               = $status['start_time'];
		$backup_data['manual_delete']             = 0;
		$backup_options                           = InstaWP_taskmanager::get_task_options( $this->task['id'], 'backup_options' );
		$lock                                     = InstaWP_taskmanager::get_task_options( $this->task['id'], 'lock' );
		...
		global $instawp_plugin;
		$backup_data['log']    = $instawp_plugin->instawp_log->log_file;
		...
		$backup_list = 'instawp_backup_list';

		$backup_list = apply_filters( 'get_instawp_backup_list_name', $backup_list, $this->task['id'] );

		$list                      = InstaWP_Setting::get_option( $backup_list );
		$list[ $this->task['id'] ] = $backup_data;
		InstaWP_Setting::update_option( $backup_list, $list );
	}
...
class InstaWP_Backup_Item {
	...
	public function get_files( $has_dir = true ) {
		...
		if ( isset( $this->config['backup']['files'] ) ) {
			...
			foreach ( $this->config['backup']['files'] as $file ) {
				...
				} //$this->get_local_path().$file['file_name'];
				else {
	...
	public function check_backup_files() {
		...
		} elseif ( isset( $this->config['backup']['data']['meta']['files'] ) ) {
			$b_has_data = true;
			...
			foreach ( $tmp_data as $file ) {
				...
				if ( file_exists( $path ) ) {
					if ( filesize( $path ) == $file['size'] ) {
						if ( $instawp_plugin->instawp_check_zip_valid() ) {
							$res = true;
						} else {
							$res = false;
						}
	...
	public function get_local_path() {
		...
		$path = apply_filters( 'instawp_get_site_instawp_path', $path, $this->config['local']['path'] );
	...
	public function get_local_url() {
		...
		$url = apply_filters( 'instawp_get_site_instawp_url', $url, $this->config['local']['path'] );
...
//				}
		...
		} elseif ( isset( $this->config['backup']['data'] ) ) {
			if ( isset( $this->config['backup']['ismerge'] ) && $this->config['backup']['ismerge'] == 1 ) {
				...
				$packages[ $index ]['option']['root_flag'] = INSTAWP_BACKUP_ROOT_WP_CONTENT;
				foreach ( $this->config['backup']['data']['meta']['files'] as $file ) {
					$packages[ $index ]['files'][] = $file['file_name'];
				}
				$index ++;
			...
			foreach ( $this->config['backup']['data']['type'] as $type ) {
				if ( $type['type_name'] == 'backup_db' ) {
					...
					$packages[ $index ]['option']['root_flag'] = INSTAWP_BACKUP_ROOT_CUSTOM;
				} elseif ( $type['type_name'] == 'backup_themes' ) {
					//$packages[$index]['option']['root']='wp-content';
					$packages[ $index ]['option']['root_flag'] = INSTAWP_BACKUP_ROOT_WP_CONTENT;
				} elseif ( $type['type_name'] == 'backup_plugin' ) {
					//$packages[$index]['option']['root']='wp-content';
					$packages[ $index ]['option']['root_flag'] = INSTAWP_BACKUP_ROOT_WP_CONTENT;
				} elseif ( $type['type_name'] == 'backup_uploads' ) {
					//$packages[$index]['option']['root']='wp-content';
					$packages[ $index ]['option']['root_flag'] = INSTAWP_BACKUP_ROOT_WP_CONTENT;
				} elseif ( $type['type_name'] == 'backup_content' ) {
					//$packages[$index]['option']['root']='';
					$packages[ $index ]['option']['root_flag'] = INSTAWP_BACKUP_ROOT_WP_ROOT;
				} elseif ( $type['type_name'] == 'backup_core' ) {
					//$packages[$index]['option']['root']='';
					$packages[ $index ]['option']['root_flag']      = INSTAWP_BACKUP_ROOT_WP_ROOT;
	...
	public static function get_backup_files( $backup ) {
		...
		uasort( $files, function ( $a, $b ) {
			...
			$index_1     = 0;
			if ( preg_match( '/instawp-.*_.*_.*\.part.*\.zip$/', $file_name_1 ) ) {
			...
			$index_2 = 0;
			if ( preg_match( '/instawp-.*_.*_.*\.part.*\.zip$/', $file_name_2 ) ) {
	...
	public function get_download_progress( $backup_id, $files ) {
		...
		foreach ( $files as $file ) {
				...
				if ( empty( $this->config['remote'] ) ) {
					...
					if ( $task === false ) {
						...
						$ret['files'][ $file['file_name'] ]['html']   = '<div style="float:left;margin:10px 10px 10px 0;text-align:center; width:180px;">
                                                                  ...
                                                                  <span id=\'' . $backup_id . '-text-part-' . $file_part_num . '\'><a onclick="instawp_prepare_download(\'' . $file_part_num . '\', \'' . $backup_id . '\', \'' . $file['file_name'] . '\');" style="cursor: pointer;">Prepare to Download</a></span></br>
                                                                  ...
                                                                  <span>size:</span><span>' . $instawp_plugin->formatBytes( $file['size'] ) . '</span>
                                                                  </div>';
					} else {
						$ret['result'] = INSTAWP_SUCCESS;
						if ( $task['status'] === 'running' ) {
							...
							$ret['files'][ $file['file_name'] ]['html']   = '<div style="float:left;margin:10px 10px 10px 0;text-align:center; width:180px;">
                                                                 <span>Part' . $format_part . '</span></br>
                                                                 <span id=\'' . $backup_id . '-text-part-' . $file_part_num . '\'><a onclick="instawp_download(\'' . $backup_id . '\', \'' . $check_type . '\', \'' . $file['file_name'] . '\');" style="cursor: pointer;">Download</a></span></br>
			...
			$ret['files'][ $file['file_name'] ]['size'] = $instawp_plugin->formatBytes( $file['size'] );
			$file_count ++;
			$file_part_num ++;
		}
		if ( $file_count % 2 != 0 ) {
	...
	public function cleanup_remote_backup() {
		if ( ! empty( $this->config['remote'] ) ) {
			...
			foreach ( $this->config['remote'] as $remote ) {
				InstaWP_downloader::delete( $remote, $files );
			}
		}
	}
}

class InstaWP_Backup {
	public $task;
	...
	public function __construct( $task_id = false, $task = false ) {
		if ( $task_id !== false ) {
			$this->task = new InstaWP_Backup_Task( $task_id );
		} elseif ( $task !== false ) {
			$this->task = new InstaWP_Backup_Task( false, $task );
		} else {
			$this->task = new InstaWP_Backup_Task();
		}
		// $bkp_init_opt = get_option('instawp_backup_init_options', '');
		// if (!empty($bkp_init_opt)) {
		//    $this->task = InstaWP_taskmanager::get_task($task_id);
	...
	public function backup( $task_id ) {
		...
		while ( $next_backup !== false ) {
			...
			if ( isset( $next_backup['files'] ) ) {
				$instawp_plugin->instawp_log->WriteLog( 'File number: ' . sizeof( $next_backup['files'] ), 'notice' );
			}
			$result = $this->_backup( $next_backup );
			$instawp_plugin->instawp_log->WriteLog( 'Backing up ' . $next_backup['key'] . ' completed.', 'notice' );
			$this->task->update_sub_task_progress( $next_backup['key'], 1, sprintf( __( 'Backing up %s finished.', 'instawp-connect' ), $next_backup['key'] ) );
			$this->task->update_backup_result( $next_backup, $result );
			$instawp_plugin->check_cancel_backup( $task_id );
			unset( $next_backup );
			$next_backup = $this->task->get_need_backup();
		}

		InstaWP_Setting::update_option( 'instawp_backup_report', $this->backup_type_report );
		$this->cleanup();
...
//		update_option( 'aaa_check', $aaa_check );
		...
		$is_type_db = false;
		$is_type_db = apply_filters( 'instawp_check_type_database', $is_type_db, $data );
		if ( $is_type_db ) {
			include_once INSTAWP_PLUGIN_DIR . '/includes/class-instawp-backup-database.php';
			$instawp_plugin->instawp_log->WriteLog( 'Start exporting database.', 'notice' );
			$backup_database = new InstaWP_Backup_Database();
			$result          = $backup_database->backup_database( $data, $this->task->get_id() );
			$instawp_plugin->instawp_log->WriteLog( 'Exporting database finished.', 'notice' );
			if ( $result['result'] == INSTAWP_SUCCESS ) {
		...
		if ( isset( $data['resume_packages'] ) ) {
			...
			$is_additional_db = apply_filters( 'instawp_check_additional_database', $is_additional_db, $data );
			if ( $is_additional_db ) {
	...
	public function cleanup() {
		...
		foreach ( $files as $file ) {
			if ( file_exists( $file ) ) {
				global $instawp_plugin;
				$instawp_plugin->instawp_log->WriteLog( 'Cleaned up file, filename: ' . $file, 'notice' );
				@unlink( $file );
			}
		}
	}
	...
	public function clean_backup() {
		...
		if ( $handler !== false ) {
			while ( ( $filename = readdir( $handler ) ) !== false ) {
				if ( preg_match( '#' . $this->task->get_id() . '#', $filename ) || preg_match( '#' . apply_filters( 'instawp_fix_instawp_free', $this->task->get_id() ) . '#', $filename ) ) {
