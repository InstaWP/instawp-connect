...
class InstaWP_restore_data
{
            ...
            if ( $has_child ) {
                foreach ( $task['files'] as $file ) {
                    ...
                    foreach ( $temp_files as $delete_file ) {
                        $path = $instawp_plugin->get_backup_path($backup_item, $delete_file['file_name']);
    ...
    public function init_restore_data( $backup_id,$restore_options=array() ) {
        ...
        $data['status'] = INSTAWP_RESTORE_INIT;
        $data['error'] = '';
        $data['error_task'] = '';
        ...
        foreach ( $packages as $index => $package ) {
            ...
            $data['restore_tasks'][ $index ]['status'] = INSTAWP_RESTORE_WAIT;
    ...
    public function get_restore_error() {
        ...
        if ( empty( $this->restore_cache) ) {
            return array(
				'task_name' => 'file', 
				'error'     => 'Restore data file not found, it may be deleted. Please verify the file exists.',
			);
        }
        else {
            return  array(
				'task_name' => $this->restore_cache['error_task'], 
				'error'     => $this->restore_cache['error'],
			);
        }
    ...
    public function save_error_log_to_debug() {
        ...
        InstaWP_error_log::create_restore_error_log($this->restore_log->log_file);
    }
    ...
    public function update_error( $error,$error_task='',$table=array() ) {
        ...
        if ( $error_task != '' ) {
            ...
            $this->restore_cache['data'][ $error_task ]['status'] = INSTAWP_RESTORE_ERROR;
    ...
    public function update_sub_task( $task_index,$result ) {
        ...
        if ( $result['result'] == INSTAWP_SUCCESS ) {
            ...
            if ( empty($files) ) {
                ...
                $this->restore_cache['restore_tasks'][ $task_index ]['result'] = $result;
                $this->restore_cache['restore_tasks'][ $task_index ]['status'] = INSTAWP_RESTORE_WAIT;
            }
        }
        else {
            $this->restore_cache['restore_tasks'][ $task_index ]['result'] = $result;
