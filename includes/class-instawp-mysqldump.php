...
class InstaWP_Mysqldump {
	...
	public function __construct(
		...
		$dumpSettingsDefault = array(
			...
			'databases'                  => false,
			'default-character-set'      => InstaWP_Mysqldump::UTF8,
			'disable-keys'               => true,
		...
		$this->host         = $host;
		$this->dbName       = $dbname;
		$this->dumpSettings = wp_parse_args( $dumpSettings, $dumpSettingsDefault );
...
//		$this->dumpSettings = self::array_replace_recursive( $dumpSettingsDefault, $dumpSettings );
		...
		if ( ! is_array( $this->dumpSettings['include_tables'] ) || ! is_array( $this->dumpSettings['exclude_tables'] ) ) {
			throw new Exception( "Include-tables and exclude_tables should be arrays" );
		}

		// Dump the same views as tables, mimic mysqldump behaviour
		$this->dumpSettings['include-views'] = $this->dumpSettings['include_tables'];
	...
	public function start( $filename = '' ) {
		...
		// If there still are some tables/views in include_tables array,
		// that means that some tables or views weren't found.
		// Give proper error and exit.
		// This check will be removed once include_tables supports regexps
		if ( 0 < count( $this->dumpSettings['include_tables'] ) ) {
			$name = implode( ",", $this->dumpSettings['include_tables'] );
		...
		if($this -> privileges['SHOW VIEW'] == 0){
			$instawp_plugin->instawp_log->WriteLog('The lack of SHOW VIEW privilege, the backup will skip exportViews() to continue.','notice');
		}else{
			$this->exportViews();
		}

		if($this -> privileges['TRIGGER'] == 0){
			$instawp_plugin->instawp_log->WriteLog('The lack of TRIGGER privilege, the backup will skip exportTriggers() to continue.','notice');
		}else{
			$this->exportTriggers();
		}

		if($this -> privileges['CREATE ROUTINE'] == 0){
			$instawp_plugin->instawp_log->WriteLog('The lack of CREATE ROUTINE privilege, the backup will skip exportProcedures() to continue.','notice');
		}else{
			$this->exportProcedures();
		}

		if($this -> privileges['EVENT'] == 0){
			$instawp_plugin->instawp_log->WriteLog('The lack of EVENT privilege, the backup will skip exportEvents() to continue.','notice');
		}else{
			$this->exportEvents();
		}
		*/
	...
	private function getDatabaseStructure() {
		...
		if ( empty( $this->dumpSettings['include_tables'] ) ) {
			...
			// include only the tables mentioned in include_tables
			foreach ( $this->query( $this->typeAdapter->show_tables( $this->dbName ) ) as $row ) {
				if ( in_array( current( $row ), $this->dumpSettings['include_tables'], true ) ) {
					array_push( $this->tables, current( $row ) );
					$elem = array_search(
						current( $row ),
						$this->dumpSettings['include_tables']
					);
					unset( $this->dumpSettings['include_tables'][ $elem ] );
				}
			}
		}
	...
	private function exportTables() {
		...
		if ( $this->task_id !== '' ) {
			...
			$options = InstaWP_taskmanager::get_task_options( $this->task_id, $options_name );
		...
		foreach ( $this->tables as $table ) {
			...
			// if this table is in exclude list.
			if ( in_array( $table, $this->dumpSettings['exclude_tables'] ) ) {
				continue;
			}
	...
	private function exportViews() {
		if ( false === $this->dumpSettings['no-create-info'] ) {
			...
			foreach ( $this->views as $view ) {
				if ( $this->matches( $view, $this->dumpSettings['exclude_tables'] ) ) {
					continue;
				}
	...
	private function listValues( $tableName ) {
		...
		if ( in_array( $tableName, $this->dumpSettings['exclude_tables_data'] ) ) {
			return;
		}
		...
		// Faker initialize only when user checked the anonymize_option
		$faker = null;
