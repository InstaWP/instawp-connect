...
class InstaWP_Setting {
	...
	public static function get_stages() {
		$stages = array(
			...
			'inventory-installation-started'   => esc_html__( 'Plugins and themes installation started.', 'instawp-connect' ),
			'inventory-installation-completed' => esc_html__( 'Plugins and themes installation completed.', 'instawp-connect' ),
...
//          'failed'                   => esc_html__( 'Migration is failed', 'instawp-connect' ),
	...
	public static function generate_section_field( $field = array() ) {
		...
		switch ( $field_type ) {
			...
			case 'email':
			case 'url':
				$css_class = 'inline-block rounded-md border-grayCust-350 shadow-sm focus:border-secondary focus:ring-1 focus:ring-secondary sm:text-sm';
